...
def read_trigger_config(filename):
    ...
    # map the name to the trigger class
    func_dict = {'TITLE': TitleTrigger,
                 ...
                 'AND': AndTrigger,
                 'OR': OrTrigger}
    trigger_dict = {}  # map all created trigger
    trigger_list = []  # store the added trigger
    # for each line in lines
    for line in lines:
        # split the string by comma, append it in a list called splitlines
        split = line.split(',')
        # if split[0] is not ADD and not AND
        if split[0] != 'ADD' and split[1] != 'AND':
            # create instances with split[2] as arg
            trigger_dict[split[0]] = func_dict[split[1]](split[2])
        # if split[1] is AND
        if split[1] == 'AND':
            # create instances with split[2] and split[3] as arg
            trigger_dict[split[0]] = func_dict[split[1]](trigger_dict[split[2]], trigger_dict[split[3]])
        # else if split[0] is ADD
        if split[0] == 'ADD':
            # append split[1:] to trigger_list
            for s in split[1:]:
                trigger_list.append(trigger_dict[s])
...
def main_thread(master):
    ...
    try:
        # t1 = TitleTrigger("traders")
        # t2 = DescriptionTrigger("Health")
